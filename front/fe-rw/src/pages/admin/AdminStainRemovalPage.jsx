...
const AdminStainRemovalPage = () => {
  // 상태 관리
  const [stainRemovals, setStainRemovals] = useState([]);
  const [stainTypes, setStainTypes] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentStainRemoval, setCurrentStainRemoval] = useState(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedStainType, setSelectedStainType] = useState("");
  const [formData, setFormData] = useState({
    stain: "",
    method: "",
    updatedMethod: "", // PATCH 요청용
  });
  ...
  const API_URL = useProxy
    ? PROXY_API.ADMIN_STAIN_REMOVAL.GET_ALL
    : ADMIN_STAIN_REMOVAL_API.GET_ALL;

  const CREATE_URL = useProxy
    ? PROXY_API.ADMIN_STAIN_REMOVAL.CREATE
    : ADMIN_STAIN_REMOVAL_API.CREATE;

  const DELETE_URL = useProxy
    ? PROXY_API.ADMIN_STAIN_REMOVAL.DELETE
    : ADMIN_STAIN_REMOVAL_API.DELETE;

  const UPDATE_URL = useProxy
    ? PROXY_API.ADMIN_STAIN_REMOVAL.UPDATE
    : ADMIN_STAIN_REMOVAL_API.UPDATE;
  ...
  const filteredStainRemovals = stainRemovals.filter((item) => {
    const matchesSearch =
      item.stain.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.method.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesStainType = selectedStainType
      ? item.stain === selectedStainType
      : true;
    return matchesSearch && matchesStainType;
  });
  ...
  // 모달 열기 - 항목 수정
  const openEditModal = (stainRemoval) => {
    setCurrentStainRemoval(stainRemoval);
    setFormData({
      stain: stainRemoval.stain,
      method: stainRemoval.method,
      updatedMethod: stainRemoval.method,
    });
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (stainInfo) => {
    if (window.confirm("정말로 이 항목을 삭제하시겠습니까?")) {
      try {
        console.log("얼룩 세탁법 삭제 요청 데이터:", {
          stain: stainInfo.stain,
          method: stainInfo.method,
        });
        ...
        console.log("얼룩 세탁법 삭제 응답:", response.data);

        if (response.data && response.data.success) {
          alert("얼룩 세탁법이 성공적으로 삭제되었습니다.");
          fetchStainRemovals(); // 데이터 새로고침
        } else {
          throw new Error(
            response.data.error?.message || "얼룩 세탁법 삭제에 실패했습니다."
          );
        }
      } catch (err) {
        console.error("얼룩 세탁법 삭제 중 오류 발생:", err);
        alert("오류가 발생했습니다: " + handleApiError(err));
      }
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedStainType(e.target.value)}
                >
                  <option value="">모든 얼룩 종류</option>
                  {stainTypes.map((type) => (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredStainRemovals.length > 0 ? (
                    filteredStainRemovals.map((stainRemoval) => (
                      <tr key={stainRemoval.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-xl text-gray-500">
                          {stainRemoval.created_at}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-xl text-gray-500">
                          {stainRemoval.updated_at}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-xl font-medium">
                          <button
                            onClick={() => openEditModal(stainRemoval)}
                            className="text-blue-600 hover:text-blue-900 mr-3"
                          >
                            수정
                          </button>
                          <button
                            onClick={() => handleDelete(stainRemoval)}
            ...
            <nav className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200">
              <div className="flex-1 flex justify-between items-center">
                ...
                <span className="text-lg text-gray-700">
                  전체{" "}
                  <span className="font-medium">{stainRemovals.length}</span>{" "}
                  항목 중{" "}
                  <span className="font-medium">
                    {filteredStainRemovals.length}
                  </span>{" "}
                  항목 표시
                </span>
                <button
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        readOnly
                        className="w-full border rounded-md p-3 text-lg bg-gray-100"
                      />
                    </div>
                    <div>
                      <label className="block text-lg font-medium text-gray-700 mb-2">
                      ...
                      <textarea
                        name="updatedMethod"
                        value={formData.updatedMethod}
                        onChange={handleInputChange}
                        className="w-full border rounded-md p-3 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        required
                        rows="4"
                        placeholder="세탁 방법을 자세히 입력해주세요."
                      ></textarea>
                    </div>
                  </>
                ) : (
...
export default AdminStainRemovalPage;
